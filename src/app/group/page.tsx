  ...
  if (loading) {
    return (
      <div className="min-h-screen bg-gunmetal pt-32 pb-20 flex items-center justify-center">
        <div className="text-pubg-yellow font-bold uppercase tracking-widest animate-pulse">Loading Group...</div>
      </div>
    );
  }
    ...
    <div className="min-h-screen bg-gunmetal pt-32 pb-20">
      <div className="container mx-auto px-4 max-w-4xl">
        <motion.div
          ...
          className="bg-[#111] border border-pubg-yellow/20 rounded-2xl p-6 md:p-8 shadow-2xl relative overflow-hidden"
        >
          {/* Background Glow */}
          <div className="absolute top-0 left-1/2 -translate-x-1/2 w-[500px] h-[500px] bg-pubg-yellow/5 rounded-full blur-[100px] pointer-events-none" />

          <div className="relative z-10">
            <div className="flex items-center justify-between mb-8 border-b border-white/10 pb-6">
              <div>
                <h1 className="text-3xl font-black font-heading uppercase text-white flex items-center gap-3">
                  <MessageSquare className="w-8 h-8 text-pubg-yellow" /> Tournament Group
                </h1>
              ...
              <div className="space-y-6 max-h-[70vh] overflow-y-auto pr-4 custom-scrollbar">
                {announcements.map((announcement) => (
                  <motion.div 
                    ...
                    key={announcement.id} 
                    className="bg-white/5 border border-white/10 rounded-xl p-6 relative overflow-hidden group hover:border-pubg-yellow/30 transition-colors shadow-lg"
                  >
                    <div className="absolute top-0 left-0 w-1.5 h-full bg-pubg-yellow"></div>
                    <div className="flex justify-between items-start mb-4">
                      <span className="text-pubg-yellow font-black uppercase tracking-widest text-sm flex items-center gap-2">
                    ...
                    {(announcement.room_id || announcement.room_password) && (
                      <div className="grid grid-cols-1 sm:grid-cols-2 gap-4 mt-4">
                        <div className="bg-black/80 p-4 rounded-lg border border-white/10 flex items-center justify-between group/copy hover:border-pubg-yellow/50 transition-colors">
                          <div>
                            <p className="text-white/40 text-[10px] font-bold uppercase tracking-widest mb-1">Room ID</p>
                            <p className="text-pubg-yellow font-mono text-lg font-bold">{announcement.room_id || '-'}</p>
                          </div>
                          {announcement.room_id && (
                            <button 
                              onClick={() => copyToClipboard(announcement.room_id)}
                              className="p-3 bg-white/5 hover:bg-pubg-yellow/20 rounded-lg text-white/60 hover:text-pubg-yellow transition-colors"
                              title="Copy Room ID"
                            >
